...
async def user_dashboard(request: Request):
    ...
    try:
        result = await run_in_threadpool(lambda: supabase.table("user_vocabulary").select("vocab_word_id, word_id, vocabulary_words(word, definition), created_at").eq("user_id", user["user_id"]).order("created_at", desc=True).limit(3).execute())
        ...
        if result:
            if len(result.data) > 0 and datetime.now(timezone.utc) - datetime.fromisoformat(result.data[0]["created_at"].replace("Z", "+00:00")) < timedelta(hours=24):
                ...
                new_result = await run_in_threadpool(lambda: supabase.rpc('get_unique_vocabulary_words_for_user', { "p_user_id": user["user_id"], "limit_count": 3 }).execute())

                if new_result:
                    return new_result.data

        return {
            "error": f"Failed to fetch current vocabulary words for user with id {user['user_id']}!"
        }
